...
const makeSocket = (config) => {
    ...
    const waitForSocketOpen = async () => {
        ...
        // Active Wake-up: try connect if it's closed
        if (ws.isClosed || ws.isClosing) {
        ...
        await Promise.race([
            new Promise((resolve, reject) => {
                ...
                ws.ev.on('open', onOpen)
                ws.ev.on('close', onClose)
                ws.ev.on('error', onClose)
            }),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Wait for socket timeout (15s)')), timeoutMs))
        ]).finally(() => {
            ws.ev.off('open', onOpen)
            ws.ev.off('close', onClose)
            ws.ev.off('error', onClose)
        })
    }
    ...
    const requestPairingCode = async (phoneNumber, code = "ABCD1234") => {
        ...
        // 2. Now set the identity for pairing
