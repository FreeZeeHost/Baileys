...
const makeSocket = (config) => {
        ...
        if (node.attrs.lid && authState.creds.me?.id) {
            ...
            process.nextTick(async () => {
                try {
                    ...
                    await (0, Utils_1.signalRepository).lidMapping.storeLIDPNMappings([{ lid: myLID, pn: myPN }]);
    ...
    ws.on('CB:failure', (node) => {
        const reason = +(node.attrs.reason || 500); console.log("\n[DEBUG] WhatsApp Rejection Reason:", reason, node.attrs);
