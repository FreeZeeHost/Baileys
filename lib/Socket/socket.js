...
const makeSocket = (config) => {
    ...
    const validateConnection = async () => {
        ...
        logger.debug({
            browser,
            helloMsg
        }, 'connected to WA (Handshake Start)')
        ...
        if (!creds.me) {
            ...
            logger.debug('Attempting registration node...')
        } else {
            node = Utils_1.generateLoginNode(creds.me.id, config)
            logger.debug('Attempting login node...')
        }
    ...
    const waitForSocketOpen = async () => {
        ...
        if (ws.isClosed || ws.isClosing) {
            logger.info('Socket was closed, reconnecting for pairing...')
            ws.connect() // Re-attempt connect if it's closed
        }
    ...
    const requestPairingCode = async (phoneNumber, code = "ABCD1234") => {
        // Wait for socket to be open but with a very short timeout
        await waitForSocketOpen()
        
        // Ensure creds are initialized for pairing
        ...
        // Force sending the pairing IQ with high priority
