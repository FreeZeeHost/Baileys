...
const makeSocket = (config) => {
    ...
    const waitForSocketOpen = async () => {
        ...
        if (ws.isClosed || ws.isClosing) {
            logger.info('Socket was closed, forcing connection for pairing...')
            ws.connect()
        }
        ...
        await Promise.race([
            new Promise((resolve, reject) => {
                onOpen = () => {
                    logger.debug('Socket opened successfully for pairing')
                    resolve(undefined)
                }
                onClose = (e) => reject(e?.error || new Error('Socket closed prematurely during pairing wait'))
                ws.on('open', onOpen)
                ws.on('close', onClose)
                ws.on('error', onClose)
            }),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Wait for socket timeout (15s)')), timeoutMs))
        ]).finally(() => {
            ws.off('open', onOpen)
            ws.off('close', onClose)
            ws.off('error', onClose)
    ...
    const requestPairingCode = async (phoneNumber, code) => {
        // 1. Ensure socket is open and validated as a NEW connection first
        // If creds.me exists, it will try to login and likely fail if not registered
        await waitForSocketOpen()
        
        // 2. Clear old session data to force a FRESH pairing
        delete authState.creds.me
        authState.creds.registered = false
        
        // 3. Set pairing code (use random if not provided)
        authState.creds.pairingCode = code?.toUpperCase() || Utils_1.bytesToCrockford(crypto_1.randomBytes(5)).substring(0, 8)
        
        const cleanedNumber = phoneNumber.replace(/\D/g, '')
        authState.creds.me = {
            id: WABinary_1.jidEncode(cleanedNumber, 's.whatsapp.net'),
            name: '~'
        }

        ev.emit('creds.update', authState.creds)

        // 4. Send pairing request with standard platform ID
        await sendNode({
            tag: 'iq',
            attrs: {
                to: WABinary_1.S_WHATSAPP_NET,
            ...
            content: [{
                ...
                content: [{
                    ...
                    },
                    {
                        ...
                        attrs: {},
                        content: '49' // Standard Chrome ID
                    },
                    {
                        tag: 'companion_platform_display',
                        attrs: {},
                        content: 'Chrome (Ubuntu)' // Matching Browsers.ubuntu('Chrome')
                    },
                    {
