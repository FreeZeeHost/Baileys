...
const makeSocket = (config) => {
    ...
    const waitForSocketOpen = async () => {
        ...
        if (ws.isClosed || ws.isClosing) {
            logger.info('Socket closed, attempting to reconnect for pairing...')
            ws.connect()
        }
        let onOpen
        let onClose
        ...
        await Promise.race([
            new Promise((resolve, reject) => {
                ...
                onClose = (e) => reject(e?.error || new boom_1.Boom('Connection Closed', { statusCode: Types_1.DisconnectReason.connectionClosed }))
    ...
    const requestPairingCode = async (phoneNumber, code = "ABCD1234") => {
        ...
        try {
            ...
            logger.warn('Proceeding with pairing despite connection delay...')
        }
