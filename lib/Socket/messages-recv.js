...
const makeMessagesRecvSocket = (config) => {
    ...
    const handleMexNotification = (id, node) => {
        ...
        if (operation === Types_1.MexOperations.UPDATE) {
            ...
            if (contentPath?.thread_metadata?.settings) {
            ...
            if (contentPath && contentPath.role_updates?.[0]) {
            ...
            if (contentPath) {
                ev.emit('newsletter-participants.update', {
                    ...
                    user: contentPath.user?.pn || contentPath.user?.jid,
    ...
    // recv a message
    ws.on('CB:broadcast', (node) => { processNode('message', node, 'processing broadcast', handleMessage) })
