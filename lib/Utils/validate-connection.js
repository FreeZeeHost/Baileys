...
const getNormalizedBrowser = (config) => {
    const browser = [...(config.browser || [])]
    if (!browser[0]) browser[0] = 'Mac OS'
    if (!browser[1]) browser[1] = 'Chrome'
    const os = browser[0]
    if (os !== 'Mac OS' && os !== 'macOS' && os !== 'Windows' && os !== 'Android' && os !== 'iOS' && os !== 'Portal' && os !== 'WearOS') {
        browser[0] = 'Mac OS'
    }
    return browser
}
...
const getUserAgent = (config) => {
    const normalizedBrowser = getNormalizedBrowser(config)
    let platform = WAProto_1.proto.ClientPayload.UserAgent.Platform.MACOS
    const os = normalizedBrowser[0]
    if (os === 'Windows') {
        platform = WAProto_1.proto.ClientPayload.UserAgent.Platform.WINDOWS
    } else if (os === 'Android' || os === 'WearOS') {
        platform = WAProto_1.proto.ClientPayload.UserAgent.Platform.ANDROID
    } else if (os === 'iOS') {
        platform = WAProto_1.proto.ClientPayload.UserAgent.Platform.IOS
    } else if (os === 'Portal') {
        platform = WAProto_1.proto.ClientPayload.UserAgent.Platform.PORTAL
    }
...
const PLATFORM_MAP = {
    ...
    'iOS': WAProto_1.proto.ClientPayload.WebInfo.WebSubPlatform.DARWIN,
    'Portal': WAProto_1.proto.ClientPayload.WebInfo.WebSubPlatform.DARWIN,
    'WearOS': WAProto_1.proto.ClientPayload.WebInfo.WebSubPlatform.WIN_HYBRID
}

const getWebInfo = (config) => {
    const normalizedBrowser = getNormalizedBrowser(config)
    const os = normalizedBrowser[0]
    const webSubPlatform = PLATFORM_MAP[os] || WAProto_1.proto.ClientPayload.WebInfo.WebSubPlatform.DARWIN
...
const generateRegistrationNode = ({ registrationId, signedPreKey, signedIdentityKey }, config) => {
    ...
    const appVersionBuf = crypto_1.createHash('md5')
        ...
        .digest()
    const companion = {
        os: normalizedBrowser[0],
        platformType: getPlatformType(normalizedBrowser[1]),
...
const configureSuccessfulPairing = (stanza, { advSecretKey, signedIdentityKey, signalIdentities }) => {
    ...
    const hmacPrefix = isHostedAccount ? Defaults_1.WA_ADV_HOSTED_ACCOUNT_SIG_PREFIX : Buffer.alloc(0) 
