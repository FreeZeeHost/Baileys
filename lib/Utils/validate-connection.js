...
const generateRegistrationNode = ({ creds }) => {
    const appVersion = [2, 3000, 1017531287]
    
    // Create Valid MD5 Build Hash
    const buildHash = (0, crypto_1.createHash)('md5').update(appVersion.join('.')).digest()

    const companion = {
        passive: true, // MUST BE TRUE for pairing stability
        userAgent: {
        ...
        devicePairingData: {
            buildHash: buildHash,
            deviceProps: Buffer.from("CgZVYnVudHUSBggKEA8YBxgBIAE=", "base64"), // Valid Ubuntu Chrome Props
