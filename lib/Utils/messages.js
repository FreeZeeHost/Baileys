...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('text' in message) {
        ...
        m.pinInChatMessage.senderTimestampMs = message.pin?.senderTimestampMs || Date.now()
        ...
        m.keepInChatMessage.timestampMs = message.keep?.timestampMs || Date.now()
        ...
        switch (message.type) {
            ...
            case 'interactive':
                m.interactiveResponseMessage = {
                    body: {
                        text: message.buttonReply.body || message.buttonReply.displayText || '',
        ...
        const stickerPromises = stickers.map(async (s, i) => {
            ...
            // Convert and resize to compliant 512x512 WebP sticker format
            ...
            try {
                ...
                if (options.logger) {
                    options.logger.error({ err }, 'Failed to convert sticker to WebP, sending raw buffer');
                }
            }

            const hash = crypto_2.sha256(processedBuffer).toString('base64url')
            const fileName = `${i.toString().padStart(2, '0')}_${hash}.webp`
            stickerData[fileName] = [new Uint8Array(processedBuffer), {
                level: 0
            }]
        ...
        // Convert and resize cover to compliant 512x512 PNG format
        let processedCoverBuffer = coverBuffer;
        try {
            const sharp = require('sharp');
            processedCoverBuffer = await sharp(coverBuffer)
                .resize(512, 512, {
                    fit: 'contain',
                    background: { r: 0, g: 0, b: 0, alpha: 0 }
                })
        ...
        const imageDataHash = crypto_2.sha256(processedCoverBuffer).toString('base64')
        ...
        m.richResponseMessage = message.richResponseMessage
    } else if ('botForwardedMessage' in message) {
    ...
    } else if ('carouselMessages' in message) {
        m.carouselMessages = message.carouselMessages
    } else if ('botFeedbackMessage' in message) {
        m.botFeedbackMessage = message.botFeedbackMessage
    } else if ('eventMessage' in message) {
        m.eventMessage = message.eventMessage
    } else if ('commentMessage' in message) {
        m.commentMessage = message.commentMessage
    } else if ('newsletterAdminInviteMessage' in message) {
        m.newsletterAdminInviteMessage = message.newsletterAdminInviteMessage
    } else if ('statusQuestionAnswerMessage' in message) {
        m.statusQuestionAnswerMessage = message.statusQuestionAnswerMessage
    } else if ('scheduledCallCreationMessage' in message) {
        m.scheduledCallCreationMessage = message.scheduledCallCreationMessage
    } else if ('lottieStickerMessage' in message) {
        m.lottieStickerMessage = message.lottieStickerMessage
    } else if ('stickerPackMessage' in message) {
        m.stickerPackMessage = message.stickerPackMessage
    } else if ('pollCreationMessageV5' in message) {
        m.pollCreationMessageV5 = {
            message: {
                pollCreationMessageV5: message.pollCreationMessageV5
            }
        }
    } else if ('pollCreationMessageV4' in message) {
        m.pollCreationMessageV4 = {
            message: {
                pollCreationMessageV4: message.pollCreationMessageV4
            }
        }
    } else if ('pollResultSnapshotMessage' in message) {
        m.pollResultSnapshotMessage = message.pollResultSnapshotMessage
    } else if ('questionResponseMessage' in message) {
        m.questionResponseMessage = message.questionResponseMessage
    } else if ('encReactionMessage' in message) {
        m.encReactionMessage = message.encReactionMessage
    } else if ('stickerSyncRmrMessage' in message) {
        m.stickerSyncRmrMessage = message.stickerSyncRmrMessage
    } else if ('callLogMesssage' in message) {
        m.callLogMesssage = message.callLogMesssage
    } else if ('messageHistoryNotice' in message) {
        m.messageHistoryNotice = message.messageHistoryNotice
    } else if ('messageHistoryBundle' in message) {
        m.messageHistoryBundle = message.messageHistoryBundle
    } else {
...
function getAggregateVotesInPollMessage({
    ...
    const opts = message?.pollCreationMessage?.options || message?.pollCreationMessageV2?.options || message?.pollCreationMessageV3?.options || message?.pollCreationMessageV4?.options || message?.pollCreationMessageV5?.options || []
