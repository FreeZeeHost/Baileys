...
exports.patchSocket = (conn) => {
    ...
    const uploadAnywhere = async (buffer) => {
        ...
        try {
            if (typeof conn.waUploadToServer === 'function') {
                ...
                const upload = await conn.waUploadToServer(tempFilePath, { mediaType: 'image', newsletter: true });
                try {
                    fs.unlinkSync(tempFilePath);
                } catch (err) {}
                if (upload && (upload.mediaUrl || upload.url)) {
                    return upload.mediaUrl || upload.url;
                }
            }
        } catch (e) {
            try {
    ...
    const prepareMediaUrl = async (input) => {
            ...
            if (input.startsWith('http://') || input.startsWith('https://')) {
                ...
                try {
                    ...
                    if (response.data) {
                        ...
                        return await uploadAnywhere(fileBuffer);
                    }
                } catch (err) {
        ...
        if (Buffer.isBuffer(input)) {
            try {
                return await uploadAnywhere(input);
            } catch (err) {
                return "";
            }
        }
                    ...
                    if (parsed && Array.isArray(parsed.sections)) {
                        for (const sec of parsed.sections) {
                            ...
                            if (vm.primitive) {
                                ...
                                if (prim.media?.url) {
                                    prim.media.url = await prepareMediaUrl(prim.media.url);
                                }
                                if (prim.url) {
                                    prim.url = await prepareMediaUrl(prim.url);
                                }
                                if (prim.thumbnail_url) {
                                    prim.thumbnail_url = await prepareMediaUrl(prim.thumbnail_url);
                                }
                            }
                            if (Array.isArray(vm.primitives)) {
                                for (const prim of vm.primitives) {
                                    ...
                                    if (prim.avatar_url) {
                                        prim.avatar_url = await prepareMediaUrl(prim.avatar_url);
                                    }
                                    if (prim.reels_url) {
                                        prim.reels_url = await prepareMediaUrl(prim.reels_url);
                                    }
                                }
                            }
                        }
