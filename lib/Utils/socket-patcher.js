...
exports.patchSocket = (conn) => {
    ...
    const uploadAnywhere = async (buffer) => {
        // Skip waUploadToServer because WhatsApp's server uploads are encrypted and the mobile client
        // cannot decrypt them in GenAIImagine/rich messages due to the absence of decryption keys in the proto structure.
        // Instead, we directly use unencrypted public hosting services.
        try {
    ...
    const prepareMediaUrl = async (input, options = {}) => {
        ...
        if (typeof input === 'string') {
            if (input.startsWith('http://') || input.startsWith('https://')) {
                // Return web URLs directly. The client can load unencrypted public URLs natively.
                // Re-uploading them is redundant and can break rendering if uploaded to WhatsApp servers.
                return input;
            }
            try {
