    ...
    conn.msg = {
        ...
        aiThinking: async (jid, description, steps = [], options = {}) => {
            ...
            const content = {
                ...
                botForwardedMessage: {
                    message: {
                        extendedTextMessage: { text: description }
                    }
                }
            };

            await conn.relayMessage(jid, content, { messageId: options.messageId || generateMessageID(), ...options });
            return { key: { remoteJid: jid, fromMe: true, id: options.messageId || "AI_THINK" }, message: content };
        },
        ...
        aiModel: async (jid, text, modelType = 1, modelName = "Llama 3", options = {}) => {
            ...
            await conn.relayMessage(jid, content, { messageId: options.messageId || generateMessageID(), ...options });
            return { key: { remoteJid: jid, fromMe: true, id: options.messageId || "AI_MODEL" }, message: content };
        },
        ...
        aiPrompts: async (jid, text, prompts = [], options = {}) => {
            ...
            await conn.relayMessage(jid, content, { messageId: options.messageId || generateMessageID(), ...options });
            return { key: { remoteJid: jid, fromMe: true, id: options.messageId || "AI_PROMPTS" }, message: content };
        },
