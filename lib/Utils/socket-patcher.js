...
exports.patchSocket = (conn) => {
    ...
    conn.sendMessage = async (jid, content, options = {}) => {
        return taskQueue.push(async () => {
            ...
            if (conn.autoTyping && (currentContent?.text || currentContent?.conversation || currentContent?.extendedTextMessage || currentContent?.interactiveMessage)) {
                await conn.sendPresenceUpdate('composing', jid).catch(() => {});
                await new Promise(r => setTimeout(r, 1000));
            } else if (conn.autoRecord && currentContent?.audio) {
                await conn.sendPresenceUpdate('recording', jid).catch(() => {});
    ...
    conn.msg = {
        ...
        productCarousel: (jid, products, options = {}) => {
            const cards = products.map(p => ({
                header: {
                    ...
                    productMessage: {
                        product: {
                            ...
                            url: p.url || "",
                            productImage: typeof p.image === 'string' ? { url: p.image, mimetype: 'image/jpeg' } : p.image
                        },
                ...
                nativeFlowMessage: p.buttons ? { buttons: p.buttons.map(b => ({ name: b.name || "quick_reply", buttonParamsJson: JSON.stringify(b.params || {}) })) } : undefined
            }));
            return conn.sendMessage(jid, {
                interactiveMessage: {
