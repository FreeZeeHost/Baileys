...
exports.patchSocket = (conn) => {
    ...
    conn.msg = {
        ...
        aiRichMessage: async (jid, submessages, options = {}) => {
            ...
            const sections = submessages.map(sm => {
                ...
                switch(sm.messageType) {
                    case 1: primitive = { images: sm.gridImageMetadata.imageUrls.map(img => ({ url: img.imageHighResUrl || img.imagePreviewUrl, thumbnail_url: img.imagePreviewUrl, image_id: randomId(), accessibility_text: "AI Image" })), grid_layout: "2x2", __typename: "GenAIImageGridUXPrimitive" }; break;
                    case 2: primitive = { text: sm.messageText, __typename: "GenAIMarkdownTextUXPrimitive" }; break;
                    case 3: primitive = { image_url: sm.imageMetadata.imageUrl.imagePreviewUrl, thumbnail_url: sm.imageMetadata.imageUrl.imagePreviewUrl, image_id: randomId(), caption: sm.imageMetadata.imageText, __typename: "GenAIImageUXPrimitive" }; break;
            ...
            const content = {
                ...
                botForwardedMessage: { message: { richResponseMessage: { messageType: 1, submessages, unifiedResponse: { data: Buffer.from(JSON.stringify(unifiedResponseData)) }, contextInfo: { forwardingScore: 1, isForwarded: true, forwardedAiBotMessageInfo: { botJid: "867051314767696@bot" }, forwardOrigin: 4, ...(options.quoted ? { stanzaId: options.quoted.key.id, participant: options.quoted.key.participant || options.quoted.key.remoteJid, quotedMessage: options.quoted.message } : {}) } } } }
            };
        ...
        // --- 🚀 CONVENIENCE NATIVE ALIASES (As requested in Doc) ---
        ...
        sendSurvey: (jid, survey, options) => conn.sendMessage(jid, { messageContextInfo: { botMetadata: { inThreadSurveyMetadata: survey } } }, options),
        sendStickerPack: (jid, pack, options) => conn.sendMessage(jid, { stickerPackMessage: pack }, options),
    ...
    conn.smsg = (m) => {
        ...
        // smsg Reply Helpers (Symmetry with Doc)
        m.replyInvoice = (data, opt) => conn.msg.sendInvoice(m.chat, data, { quoted: m, ...opt });
        m.replyOrder = (data, opt) => conn.msg.sendOrder(m.chat, data, { quoted: m, ...opt });
        m.replyComment = (text, opt) => conn.msg.sendComment(m.chat, text, m.key, { quoted: m, ...opt });
        m.replyPollResult = (votes, opt) => conn.msg.sendPollResult(m.chat, votes, { quoted: m, ...opt });
        m.replyProduct = (owner, data, opt) => conn.msg.sendProduct(m.chat, owner, data, { quoted: m, ...opt });
        m.replyLiveLocation = (lat, lng, opt) => conn.msg.sendLiveLocation(m.chat, { degreesLatitude: lat, degreesLongitude: lng }, { quoted: m, ...opt });
        m.replyCallLog = (data, opt) => conn.msg.sendCallLog(m.chat, data, { quoted: m, ...opt });
        m.pin = (dur = 86400, opt) => conn.msg.pinMessage(m.chat, { key: m.key, type: 1, senderTimestampMs: Date.now() }, opt);
        m.keep = (opt) => conn.msg.keepMessage(m.chat, { key: m.key, keepType: 1, timestampMs: Date.now() }, opt);

        // AI Reply Helpers
        m.replyTable = (title, rows, opt) => conn.msg.aiTable(m.chat, title, rows, { quoted: m, ...opt });
        m.replyCode = (lang, code, opt) => conn.msg.aiCode(m.chat, lang, code, { quoted: m, ...opt });
        m.replyReels = (text, reels, opt) => conn.msg.aiReels(m.chat, text, reels, { quoted: m, ...opt });
        m.replyGridImage = (imgs, grid, opt) => conn.msg.aiGridImage(m.chat, imgs, grid, { quoted: m, ...opt });
        m.replyInlineImage = (url, txt, align, link, opt) => conn.msg.aiInlineImage(m.chat, url, txt, align, link, { quoted: m, ...opt });
        m.replyDynamic = (url, gif, loop, opt) => conn.msg.aiDynamic(m.chat, url, gif, loop, { quoted: m, ...opt });
        m.replyLatex = (txt, exps, opt) => conn.msg.aiLatex(m.chat, txt, exps, { quoted: m, ...opt });
        m.replyMap = (lat, lng, annots, opt) => conn.msg.aiMap(m.chat, lat, lng, annots, { quoted: m, ...opt });
        m.replyThinking = (txt, steps, opt) => conn.msg.aiThinking(m.chat, txt, steps, { quoted: m, ...opt });
        m.replyModel = (txt, type, name, opt) => conn.msg.aiModel(m.chat, txt, type, name, { quoted: m, ...opt });
        m.replyPrompts = (txt, prompts, opt) => conn.msg.aiPrompts(m.chat, txt, prompts, { quoted: m, ...opt });
        m.aiFeedback = (pos, txt, opt) => conn.msg.aiFeedback(m.chat, m.key, pos, txt, opt);
        return m;
    };
