...
exports.patchSocket = (sock) => {
    ...
    sock.msg = {
        ...
        aiTable: (jid, title, rows, options = {}) => {
            ...
            return sock.msg.aiRichMessage(jid, [{
    ...
    sock.smsg = (m) => {
        ...
        // Reply Helpers
        m.reply = (text, options = {}) => sock.sendMessage(m.chat, { text }, { quoted: m, ...options });
        m.replyImage = (url, caption = "", options = {}) => sock.sendMessage(m.chat, { image: { url }, caption }, { quoted: m, ...options });
        m.replyVideo = (url, caption = "", options = {}) => sock.sendMessage(m.chat, { video: { url }, caption }, { quoted: m, ...options });
        m.replyAudio = (url, ptt = false, options = {}) => sock.sendMessage(m.chat, { audio: { url }, ptt, mimetype: 'audio/mp4' }, { quoted: m, ...options });
        m.replyDocument = (url, fileName, options = {}) => sock.sendMessage(m.chat, { document: { url }, fileName, mimetype: 'application/octet-stream' }, { quoted: m, ...options });
        m.react = (emoji) => sock.sendMessage(m.chat, { react: { text: emoji, key: m.key } });
        m.forward = (jid, options = {}) => sock.sendMessage(jid, { forward: m }, { ...options });
        ...
        if (sock.antiCall) {
            for (const call of calls) {
                if (call.status === 'offer') {
                    sock.logger.info({ callId: call.id, from: call.from }, "Anti-Call: Rejecting incoming call");
                    await sock.rejectCall(call.id, call.from);
                    await sock.sendMessage(call.from, { text: `Maaf, saya tidak menerima panggilan. Silakan kirim pesan teks saja.` });
                }
            }
        }
    });
    ...
    sock.newsletter = {
        create: (name, description) => sock.newsletterCreate(name, description),
        follow: (jid) => sock.newsletterFollow(jid),
        unfollow: (jid) => sock.newsletterUnfollow(jid),
        mute: (jid) => sock.newsletterMute(jid),
        unmute: (jid) => sock.newsletterUnmute(jid),
        update: (jid, name, description) => sock.newsletterUpdateMetadata(jid, { name, description }),
        getInfo: (jid) => sock.getNewsletterInfo(jid)
    };

    // --- 11. POLL VOTING HELPERS ---
    sock.getPollResults = async (m) => {
