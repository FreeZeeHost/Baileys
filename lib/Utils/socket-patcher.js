    ...
    conn.msg = {
        ...
        list: async (jid, title, text, footer, buttonText, sections, options = {}) => {
            ...
            if (options.image) {
                ...
                const media = await prepareWAMessageMedia({ image: options.image }, { upload: conn.waUploadToServer }).catch(() => null);
                ...
                const { prepareWAMessageMedia } = require("./messages");
                const media = await prepareWAMessageMedia({ video: options.video }, { upload: conn.waUploadToServer }).catch(() => null);
                ...
                const { prepareWAMessageMedia } = require("./messages");
                const media = await prepareWAMessageMedia({ document: options.document }, { upload: conn.waUploadToServer }).catch(() => null);
        ...
        carousel: async (jid, cardsList, options = {}) => {
            const preparedCards = await Promise.all((cardsList || []).map(async (card) => {
                ...
                if (card.image) {
                    const { prepareWAMessageMedia } = require("./messages");
                    const media = await prepareWAMessageMedia({ image: card.image }, { upload: conn.waUploadToServer }).catch(() => null);
                    if (media?.imageMessage) {
                        header = {
                            hasMediaAttachment: true,
                            imageMessage: media.imageMessage
                    ...
                    const { prepareWAMessageMedia } = require("./messages");
                    const media = await prepareWAMessageMedia({ video: card.video }, { upload: conn.waUploadToServer }).catch(() => null);
                    if (media?.videoMessage) {
                        header = {
                            hasMediaAttachment: true,
                            videoMessage: media.videoMessage
