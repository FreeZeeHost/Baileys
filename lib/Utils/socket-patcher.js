    ...
    conn.msg = {
        ...
        aiTable: (jid, title, rows, options = {}) => {
            const formattedRows = rows.map(r => {
                const cells = Array.isArray(r) ? r : (r.cells || r.items || []);
                return { cells: cells.map(String) };
            });
            return conn.msg.nativeTable(jid, title, formattedRows, options);
        },
        aiCode: (jid, language, code, options = {}) => {
            return conn.sendMessage(jid, { text: `\`\`\`${language}\n${code}\n\`\`\`` }, options);
        },
