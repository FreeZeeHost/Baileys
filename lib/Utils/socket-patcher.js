...
exports.patchSocket = (conn) => {
    ...
    conn.msg = {
        ...
        aiTable: (jid, title, rows, options = {}) => {
            const formattedRows = rows.map(r => ({ items: Array.isArray(r) ? r : r.items, isHeading: r.isHeading || false }));
            let fallbackText = `*${title}*\n`;
            formattedRows.forEach(r => {
                fallbackText += `| ${r.items.join(" | ")} |\n`;
            });
            return conn.msg.aiRichMessage(jid, [
                { messageType: 2, messageText: fallbackText.trim() },
                { messageType: 4, tableMetadata: { title: title, rows: formattedRows } }
            ], options);
        },
        aiCode: (jid, language, code, options = {}) => {
            const fallbackText = `\`\`\`${language}\n${code}\n\`\`\``;
            return conn.msg.aiRichMessage(jid, [
                { messageType: 2, messageText: fallbackText },
        ...
        aiMap: (jid, lat, lng, annotations = [], options = {}) => {
            let fallbackText = `📍 *Lokasi Peta*\nKoordinat: ${lat}, ${lng}\n`;
            const formattedAnnots = annotations.map((a, i) => {
                fallbackText += `\n📌 *${a.title || `Titik ${i+1}`}*\n${a.body || ""}\n`;
                return { annotationNumber: i + 1, latitude: a.lat, longitude: a.lng, title: a.title || "", body: a.body || "" };
            });
            fallbackText += `\nGoogle Maps: https://maps.google.com/maps?q=${lat},${lng}`;
            return conn.msg.aiRichMessage(jid, [
                { messageType: 2, messageText: fallbackText.trim() },
                { messageType: 7, mapMetadata: { centerLatitude: lat, centerLongitude: lng, annotations: formattedAnnots, showInfoList: true } }
            ], options);
        },
