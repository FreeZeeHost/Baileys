...
const useMongoFileAuthState = async (collectionOrUrl) => {
    ...
    if (typeof targetUrl === 'string') {
        ...
        try {
            ...
            collection = client.db('freezee-baileys').collection('auth')
            console.log("\x1b[32m[✓] MongoDB Connected Successfully to: " + client.db('freezee-baileys').databaseName + "\x1b[0m")
        } catch (e) {
            console.error("\x1b[31m[!] MongoDB Connection Failed: " + e.message + "\x1b[0m")
            console.log("\x1b[33m[!] Falling back to local file state...\x1b[0m")
    ...
    const creds = (await readData('creds')) || ({ ...auth_utils_1.initAuthCreds(), registrationId: 2266 })
    
    // Force save initial creds (especially registrationId)
