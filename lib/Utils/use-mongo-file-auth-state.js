...
const useMongoFileAuthState = async (collectionOrUrl) => {
    ...
    // Auto-Connect if URL is provided (Simple One-Liner)
    if (typeof collectionOrUrl === 'string') {
        const { MongoClient } = require('mongodb')
        const url = collectionOrUrl.includes('://') ? collectionOrUrl : Buffer.from(collectionOrUrl, 'base64').toString('utf-8')
    ...
    if (!collection && process.env.MONGO_URL) {
        const { MongoClient } = require('mongodb')
