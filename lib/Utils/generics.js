...
const generateMessageID = () => 'FREEZEE' + (0, crypto_1.randomBytes)(12).toString('hex').toUpperCase()
...
const showLoading = async (message, duration) => {
    const steps = 20
...
module.exports = {
    ...
    extractMessageContent: (content) => {
        ...
        if (content?.templateMessage) {
            return content.templateMessage.hydratedTemplate || content.templateMessage.hydratedFourRowTemplate
        }
        if (content?.hydratedTemplateMessage) {
            return content.hydratedTemplateMessage.hydratedTemplate
        }
        return content
    }
}
