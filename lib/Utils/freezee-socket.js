...
exports.makeFreeZeeSocket = (config = {}) => {
    ...
    // Setup proxy for keys store so we can swap it after MongoDB load
    let keysDelegator = {
    ...
    const state = config.auth || {
        creds: initAuthCreds(),
        keys: {
            get: async (type, ids) => keysDelegator.get(type, ids),
            set: async (data) => keysDelegator.set(data),
            transaction: async (block) => {
                if (typeof keysDelegator.transaction === 'function') {
                    return keysDelegator.transaction(block);
                }
                return block();
            }
        }
