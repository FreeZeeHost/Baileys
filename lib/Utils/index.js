...
export const { 
    ...
    encodeSignedDeviceIdentity, decodeSignedDeviceIdentity, NACK_REASONS,
    NO_MESSAGE_FOUND_ERROR_TEXT, encodeBigEndian, getNextPreKeys, hkdf
} = all;

export const bindWaitForConnectionUpdate = (ev) => all.bindWaitForEvent(ev, 'connection.update');

export const {
    extractDeviceJids, jidNormalizedUser, areJidsSameUser, isJidGroup, isJidUser,
    isJidBroadcast, isJidNewsletter, isLidUser, getDevice, S_WHATSAPP_NET
} = jidUtils;
...
export const {
    ...
    encryptMediaRetryRequest, getStatusCodeForMediaRetry, getWAUploadToServer,
    getUrlFromDirectPath, cleanMessage, decodeMessageNode, extractAddressingContext,
    getCallStatusFromNode, getStatusFromReceiptType
} = messages;
