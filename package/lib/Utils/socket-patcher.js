    ...
    const uploadAnywhere = async (buffer) => {
        ...
        try {
            if (typeof conn.waUploadToServer === 'function') {
                ...
                const upload = await conn.waUploadToServer(tempFilePath, { mediaType: 'image', fileEncSha256B64: fileEncSha256, newsletter: true });
                try {
    ...
    const prepareMediaUrl = async (input, options = {}) => {
        ...
        if (typeof input === 'string') {
            if (input.startsWith('http://') || input.startsWith('https://')) {
                ...
                try {
                    const response = await axios.get(input, { responseType: 'arraybuffer', timeout: 8000 });
                    if (response.data) {
                ...
                for (const sm of richMsg.submessages) {
                    if (sm.gridImageMetadata) {
                        if (sm.gridImageMetadata.gridImageUrl) {
                            sm.gridImageMetadata.gridImageUrl.imagePreviewUrl = await prepareMediaUrl(sm.gridImageMetadata.gridImageUrl.imagePreviewUrl, options);
                            sm.gridImageMetadata.gridImageUrl.imageHighResUrl = await prepareMediaUrl(sm.gridImageMetadata.gridImageUrl.imageHighResUrl, options);
                        }
                        if (Array.isArray(sm.gridImageMetadata.imageUrls)) {
                            for (const img of sm.gridImageMetadata.imageUrls) {
                                img.imagePreviewUrl = await prepareMediaUrl(img.imagePreviewUrl, options);
                                img.imageHighResUrl = await prepareMediaUrl(img.imageHighResUrl, options);
                            }
                        }
                    }
                    if (sm.imageMetadata?.imageUrl) {
                        sm.imageMetadata.imageUrl.imagePreviewUrl = await prepareMediaUrl(sm.imageMetadata.imageUrl.imagePreviewUrl, options);
                        sm.imageMetadata.imageUrl.imageHighResUrl = await prepareMediaUrl(sm.imageMetadata.imageUrl.imageHighResUrl, options);
                    }
                    if (sm.dynamicMetadata?.url) {
                        sm.dynamicMetadata.url = await prepareMediaUrl(sm.dynamicMetadata.url, options);
                    }
                    if (sm.contentItemsMetadata?.itemsMetadata) {
                        for (const item of sm.contentItemsMetadata.itemsMetadata) {
                            if (item.reelItem) {
                                item.reelItem.thumbnailUrl = await prepareMediaUrl(item.reelItem.thumbnailUrl, options);
                                item.reelItem.profileIconUrl = await prepareMediaUrl(item.reelItem.profileIconUrl, options);
                            }
                        }
                    }
                }
            ...
            if (richMsg.unifiedResponse?.data) {
                try {
                    ...
                    if (parsed && Array.isArray(parsed.sections)) {
                        for (const sec of parsed.sections) {
                            ...
                            if (vm.primitive) {
                                ...
                                if (prim.media?.url) {
                                    prim.media.url = await prepareMediaUrl(prim.media.url, options);
                                }
                                if (prim.url) {
                                    prim.url = await prepareMediaUrl(prim.url, options);
                                }
                                if (prim.thumbnail_url) {
                                    prim.thumbnail_url = await prepareMediaUrl(prim.thumbnail_url, options);
                                }
                            }
                            if (Array.isArray(vm.primitives)) {
                                for (const prim of vm.primitives) {
                                    ...
                                    if (prim.avatar_url) {
                                        prim.avatar_url = await prepareMediaUrl(prim.avatar_url, options);
                                    }
                                    if (prim.reels_url) {
                                        prim.reels_url = await prepareMediaUrl(prim.reels_url, options);
                                    }
                                }
                            }
                        }
                    ...
                    if (typeof richMsg.unifiedResponse.data === 'string') {
                        richMsg.unifiedResponse.data = JSON.stringify(parsed);
                    } else {
                        richMsg.unifiedResponse.data = parsed;
                    }
                } catch (err) {
                    conn.logger.error({ err }, "Error patching unifiedResponse.data in relayMessage");
                }
            }
        }
    ...
    conn.msg = {
        ...
        aiRichMessage: async (jid, submessages, options = {}) => {
            ...
            for (const sm of submessages) {
                ...
                if (sm.dynamicMetadata?.url) sm.dynamicMetadata.url = await prepareMediaUrl(sm.dynamicMetadata.url, options);
                if (sm.contentItemsMetadata?.itemsMetadata) {
                    for (const item of sm.contentItemsMetadata.itemsMetadata) {
                        if (item.reelItem) {
                            item.reelItem.thumbnailUrl = await prepareMediaUrl(item.reelItem.thumbnailUrl, options);
